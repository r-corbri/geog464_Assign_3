...
//fetch("../mvp_proj_data/Sonoma_Fire_Risk_Index_web_4326.tif")    // General Lifeforms (kinda looks better)
    ...
    //const min = georaster.mins[0];   // Leaving this here in case I want to do continuos
    //const max = georaster.maxs[0];

    /* // Define color ramp for calculated risk (continuous gradient)
    ...
    function interpolateRiskColor(value) {
        ...
        // Nonlinear scaling to emphasize high risk
        ratio = Math.pow(ratio, 1.5);

        let r, g, b, a;

        if (ratio < 0.25) {
            // Low → Moderate (Green → Yellow)
            r = Math.round(255 * (ratio / 0.50)); // 0 → 255
            g = 255;
            b = 0;
            a = 0.0;
        } else if (ratio < 0.90) {
            // Moderate → High (Yellow → Orange)
            r = 255;
            g = Math.round(255 * (1 - (ratio - 0.45) / 0.35)); // 255 → 0
            b = 0;
            a = 0.6;
        } else {
            // High → Very High (Red → Dark Red)
            r = 255;
            g = Math.round(100 * (1 - (ratio - 0.9) / 0.2)); // fade to dark
            b = 0;
            a = 0.6;
        }
